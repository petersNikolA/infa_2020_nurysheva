...
def bird(x_step, y_step, scale_x, scale_y, width_of_line):
    ...
    '''
    arc(display, White, [100 + x_step, 40 + y_step, scale_x, scale_y], 0, 3.14, width_of_line)
    arc(display, White, [100 + scale_x + x_step, 40 + y_step, scale_x, scale_y], 0, 3.14, width_of_line)
...
def gull_body(surface, x, y, scale, color, width_of_line):
    polygon(surface, color, [(x + 2 * scale, y), (x + 4 * scale, y + scale),
                            (x + 8 * scale, y + 2 * scale), (x + 9 * scale, y + 6 * scale),
                            (x + 11 * scale, y + 7 * scale), (x + 13 * scale, y + 7 * scale),
                            (x + 13 * scale, y + 8 * scale), (x + 12 * scale, y + 9 * scale),
                            (x + 6 * scale, y + 9 * scale), (x + 3 * scale, y + 10 * scale),
                            (x + 2 * scale, y + 7 * scale), (x + 3 * scale, y + 6 * scale),
                            (x + 6 * scale, y + 7 * scale), (x + 5 * scale, y + 6 * scale),
                            (x + 5 * scale, y + 5 * scale), (x + 2 * scale, y + 4 * scale),
                            (x, y + 2 * scale), (x + 2 * scale, y + 3 * scale),
                            (x + 6 * scale, y + 3 * scale), (x + 8 * scale, y + 6 * scale),
                            (x + 6 * scale, y + 3 * scale), (x + 4 * scale, y + 2 * scale)], width_of_line)

def gull_head(surface, x, y, scale, width_of_line):
    ellipse(surface, White, (12 * scale + x, 6 * scale + y, 4*scale, 2*scale))
    arc(surface, Black, (12 * scale + x, 6 * scale + y, 4*scale, 2*scale), -2.1, 3.14, width_of_line)

def gull_feet(surface, x, y, scale, color, width_of_line):
    polygon(surface, color, [(12 * scale + x, 12 * scale + y), (11 * scale + x, 13 * scale + y), 
                            (12 * scale + x, 12.5 * scale + y), (13 * scale + x, 13 * scale + y), 
                            (12.5 * scale + x, 12.3 * scale + y), (13.5 * scale + x, 13 * scale + y), 
                             (13 * scale + x, 12.3 * scale + y), (14 * scale + x, 12 * scale + y)], width_of_line)
    polygon(surface, color, [(11 * scale + x, 13 * scale + y), (10 * scale + x, 14 * scale + y), 
                            (11 * scale + x, 13.5 * scale + y), (12 * scale + x, 14 * scale + y), 
                            (11.5 * scale + x, 13.3 * scale + y), (12.5 * scale + x, 14 * scale + y), 
                            (12 * scale + x, 13.3 * scale + y), (13 * scale + x, 13 * scale + y)], width_of_line) 
                                                                           
def gull_beak(surface, x, y, scale, width_of_line):
    polygon(surface, (255, 100, 100), [(16 * scale + x, 6.9 * scale + y), (18 * scale + x, 6.3 * scale + y), 
                                       (18.5 * scale + x, 7 * scale + y), (18 * scale + x, 7.7 * scale + y),
                                       (16 * scale + x, 7.1 * scale + y)])
    polygon(surface, (255, 0, 0), [(16 * scale + x, 6.9 * scale + y), (18 * scale + x, 6.3 * scale + y), 
                                   (18.5 * scale + x, 7 * scale + y), (18 * scale + x, 7.7 * scale + y),
                                   (16 * scale + x, 7.1 * scale + y)], width_of_line)
    line(surface, (255, 0, 0), (16 * scale + x, 7 * scale + y), (18.5 * scale + x, 7 * scale + y))

def gull_stomache(surface, x, y, scale, color, width_of_line):
    ellipse(surface, color, (6 * scale + x, 8 * scale + y, 6 * scale, 2 * scale))
    arc(surface, Black, (6 * scale + x, 8 * scale + y, 6 * scale, 2 * scale), 3.14, 6.28, width_of_line)

def gull_legs(surface, x, y, scale, color, width_of_line):
    polygon(surface, color, [(8 * scale + x, 10 * scale + y), (8 * scale + x, 11 * scale + y), 
                             (9 * scale + x, 13 * scale + y), (12 * scale + x, 13 * scale + y), 
                             (9 * scale + x, 12 * scale + y), (9 * scale + x, 11 * scale + y), 
                             (11 * scale + x, 12 * scale + y), (13 * scale + x, 12 * scale + y), 
                             (10 * scale + x, 11 * scale + y), (10 * scale + x, 10 * scale + y)], width_of_line)
...
def gull_anotherpart(surface, x, y, scale):
    polygon(surface, White,[(8 * scale + x, 10.2 * scale + y), (10 * scale + x, 10.2 * scale + y), 
                            (10 * scale + x, 9.9 * scale + y), (8 * scale + x, 9.9 * scale + y),
...
def gull(x, y, scale, width_of_line, eye_radius, mirror):
    ...
    mirror - reverse parametr (1 - reversed gull)
    '''
    if mirror == 1:
        surface = pygame.Surface((x + 19 * scale, y + 14 * scale), pygame.SRCALPHA)
    else:
        surface = display
    ...
    gull_stomache(surface, x, y, scale, White, width_of_line)
